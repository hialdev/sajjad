...
@section('content')
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Tipe Produk</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('home') }}">Dashboard</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Semua Tipe Produk</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
    ...
    <div class="mb-3 d-flex align-items-center gap-2 justify-content-between">
        ...
        <div style="aspect-ratio:1/1; width:3em; height:3em"
            ...
            {{ count($product_types) }}</div>
        <a href="{{ route('product_type.add') }}" class="btn btn-primary btn-al-primary">Tambah</a>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{route('product_type.index')}}" method="GET">
                <div class="row align-items-end mb-3 flex-wrap">
                    ...
                    <div class="col-md-3 mb-2">
                        ...
                        <select name="field" id="field" class="form-select">
                            @foreach (getModelAttributes('ProductType', ['pack_id', 'image', 'slug', 'unit_id']) as $atr)
            ...
            <div class="table-responsive">
                <table class="table border text-nowrap mb-0 align-middle">
                    <thead class="text-dark fs-4">
                        <tr>
                            <th>
                                <h6 class="fs-3 fw-semibold mb-0">Tipe Produk</h6>
                            </th>
                            <th>
                                <h6 class="fs-3 fw-semibold mb-0">Tipe</h6>
                            </th>
                            <th>
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <img src="{{ $product_type->image ? asset('/storage/'.$product_type->image) : '/assets/images/profile/user-1.jpg' }}"
                                            class="rounded-2" alt="product Image {{ $product_type->name }}" style="width: 4em" />
                                        <div class="ms-3">
                                            <div class="badge bg-secondary-subtle text-uppercase text-primary mb-2">{{ $product_type->code }}</div>
                                            <h6 class="fw-semibold mb-1" style="white-space: normal !important">{{ $product_type->name }}</h6>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="badge bg-secondary rounded-3 fw-semibold fs-2">
                                        {{ $product_type->type }}</div>
                                </td>
                                <td>
                                    <div class="d-flex flex-column align-items-start gap-2">
                                        <div class="badge bg-success-subtle text-success rounded-3 fw-semibold fs-2">Updated
                                            at
                                            : {{ $product_type->updated_at }}</div>
                                        <div class="badge bg-primary-subtle text-primary rounded-3 fw-semibold fs-2">Created
                                            at
                                            : {{ $product_type->created_at }}</div>
                                    </div>
                                </td>
                                <td>
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li>
                                                <a href="{{route('product_type.edit', $product_type->id)}}" class="dropdown-item d-flex align-items-center gap-3"><i
                                                        class="fs-4 ti ti-edit"></i>Edit</a>
                                            </li>
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#deleteModal-{{$product_type->id}}"><i
                                    ...
                                    <!-- Delete Modal -->
                                    <div id="deleteModal-{{$product_type->id}}" class="modal fade" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
                                            <div class="modal-content p-3 modal-filled bg-danger">
                                                ...
                                                <div class="modal-body" style="width: fit-content; white-space:normal">
                                                    <h5 class="mt-0 text-white">Product {{$product_type->title}} akan dihapus</h5>
