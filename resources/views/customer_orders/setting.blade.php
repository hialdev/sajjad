               ...
               <nav aria-label="breadcrumb">
                  <ol class="breadcrumb">
                     <li class="breadcrumb-item">
                        <a class="text-muted text-decoration-none" href="{{ route('customer-order.index') }}">Penjualan
                           Instan</a>
                     </li>
   ...
   <div class="">
      <div class="row">
         ...
         <div class="col-6 col-md-3 mb-4 d-flex justify-content-end align-items-start gap-2">
            <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteRequestOrder-{{ $cod->id }}"><i
      ...
      <div id="stepper-form">
                  ...
                  <div class="card-body">
                     <div class="row">
                        <div class="col-md-4">
                           ...
                           <div>
                              ...
                              <h6 class="fs-2 fw-semibold text-success mb-1" style="">
                                 {{ \Carbon\Carbon::parse($cod->date)->format('d F Y') }}
                              </h6>
                           </div>
                           <div>
                        ...
                        <div class="col-md-4">
                           ...
                           <a href="{{ route('customer.index', ['search' => $cod->customer->name]) }}" target="_blank"
                                       ...
                                       <div class="modal-body pt-0">
                                          ...
                                          @forelse ($cod->products as $codProduct)
                                             @php
                                                ...
                                                $priceBuy = $codProduct->price_buy ?? 0; // Harga beli (diambil dari cart)
                                             ...
                                             <div
                                                ...
                                                <div
                                                   ...
                                                   <img
                                                      src="{{ $codProduct?->product?->image ? '/storage/' . $codProduct?->product?->image : 'https://placehold.co/300?text=' . $codProduct?->product?->name }}"
                        ...
                        <form action="{{ route('customer-order.update', $cod->id) }}" method="POST"
                                 ...
                                 <div style="flex-grow:1">
                                    <select name="user_id" id="user_id" class="select2-normal form-select">
                                       ...
                                       @foreach ($users as $user)
                                          <option value="{{ $user->id }}" {{ $user->id == old('user_id', $cod->user_id) ? 'selected' : '' }}>
                                             {{ $user->name }}
                           ...
                           <div class="p-3 bg-primary-subtle rounded-3 mb-4">
                              ...
                              <div class="input-group">
                                 ...
                                 <div style="flex-grow:1">
                                    <select name="customer_id" id="customer_id" class="select2-normal form-select">
                                       <option value="">-- Pilih Pelanggan / Customer --</option>
                                       @foreach ($customers as $customer)
                                          <option value="{{ $customer->id }}" {{ $customer->id == old('customer_id', $cod->customer_id) ? 'selected' : '' }}>
                              ...
                              <div class="card-body p-0 h-100 d-flex flex-column">
                                 ...
                                 <div class="p-1 h-100 d-flex flex-column justify-content-between px-3">
                                    ...
                                    <div class="d-flex mt-auto align-items-center gap-2 mb-3">
                                       ...
                                       <form action="{{ route('customer-order.addCart', $cod->id) }}" method="POST"
                     ...
                     <div class="border border-2 border-dashed border-dark-subtle rounded-4 p-3">
                        ...
                        @forelse ($carts as $item)
                           @php
                              ...
                              // Tambahkan definisi $priceBuy
                              $priceBuy = $item['price_buy'] ?? 0; // Harga beli (diambil dari cart)
                           ...
                           <div id="cart-item-{{ $item['id'] }}">
                              ...
                              <div class="d-flex align-items-start gap-2">
                                 <div>
                                    ...
                                    <div class="fs-1 mt-1 text-muted stock-info" id="stock_info_{{ $item['id'] }}">Stock
                                       tersisa {{ $cart->stockCountIn() }}
                                       qty</div>
                                 </div>
                                 @if ($cart->type->type == 'meteran')
                                    <div>
                                       ...
                                       <div class="d-flex align-items-center gap-2">
                                          <input type="number" name="length[]" id="length_{{ $item['id'] }}"
                                             data-index="{{ $loop->index }}" class="form-control form-control-sm length-input"
                                             data-id="{{ $item['id'] }}"
                                             value="{{ old('length[' . $loop->index . ']', $codproduct?->length ?? ($item['length'] ?? 0)) }}"
                                             min="1" max="{{ $cart->width }}" {{ $cod->is_finished != 0 ? 'disabled' : '' }} />
                                       </div>
                                 ...
                                 <div class="flex-grow-1">
                                    ...
                                    <input type="text" name="price_sale[]" id="price_sale_{{ $item['id'] }}"
                                       ...
                                       data-id="{{ $item['id'] }}"
                                       value="{{ formatRupiah(old('price_sale[' . $loop->index . ']', $priceSale)) }}" min="0" {{ $cod->is_finished != 0 ? 'disabled' : '' }} />
                                    <div class="fs-1 mt-1 text-muted">Penetapan Harga Jual
                                       {{ formatRupiah($cart->price_per_unit) }}
                                    </div>
                                 </div>
                                 <button class="btn btn-sm btn-danger remove-cart align-self-center"
                                    data-url="{{ route('customer-order.removeCart', $cod->id) }}"
                                    data-product-id="{{ $item['id'] }}" data-reqorder-id="{{ $cod->id }}" {{ $cod->is_finished != 0 ? 'disabled' : '' }}>
                              ...
                              <div class="p-3 mt-2 bg-primary-subtle rounded-3 mb-4">
                                 <div class="">
                                    <div class="form-check form-switch">
                                       ...
                                       @endphp
                                       @if(!$isChecked)
                                          <input type="hidden" id="zeroPartner-{{$loop->index}}" name="is_from_partner[]"
                                             value="0" />
                                       @endif
                                       <input class="form-check-input check-from-partner" name="is_from_partner[]"
                                          data-item-id="{{$loop->index}}" type="checkbox" value="1"
                                          id="checkFromPartner{{$loop->index}}" data-product-id="{{ $item['id'] }}"
                                          data-index="{{ $loop->index }}"
                                          data-is-meteran="{{ $cart->type->type == 'meteran' ? '1' : '0' }}" {{ $isChecked ? 'checked' : '' }} />
                                       <label class="form-check-label fs-2" for="checkFromPartner{{$loop->index}}">Barang
                                          diambil dari Partner</label>
                                    </div>
                                 </div>

                                 <div
                                    class="mt-2 {{$cart->stockCountIn() > 0 || $codproduct?->store_id != null ? '' : 'd-none'}}"
                                    id="store_input_{{$loop->index}}">
                                    <label class="form-label fw-semibold fs-2">Mengambil Dari Toko</label>
                                    <div class="input-group">
                                       <span class="input-group-text px-6" id="basic-addon1"><i
                                             class="ti ti-building-community fs-6"></i></span>
                                       <div style="flex-grow:1">
                                          <select name="store_id[]" id="store_id_{{$loop->index}}"
                                             class="select2-normal store-select form-select"
                                             data-product-id="{{ $item['id'] }}" data-index="{{ $loop->index }}"
                                             data-is-meteran="{{ $cart->type->type == 'meteran' ? '1' : '0' }}">
                                             <option value="">-- Pilih Store --</option>
                                             @foreach ($stores as $store)
                                                @if((int) $store->getProductRemaining($item['id']) > 0)
                                                   <option value="{{$store->id}}" {{$store->id == old('store_id[' . $loop->index . ']', $codproduct?->store_id) ? 'selected' : ''}}
                                                      style="white-space:normal !important">
                                                      {{ "{$store->name}, {$store->address} - {$store->city}" }}</option>
                                                @endif
                                             @endforeach
                                          </select>
                                       </div>
                                 ...
                                 <div
                                    class="{{$cart->stockCountIn() > 0 || $codproduct?->store_id != null ? 'd-none' : ''}} mt-2"
                                    id="partner_input_{{$loop->index}}">
                                    <label class="form-label fw-semibold fs-2">Mengambil Dari Partner</label>
                                    <div class="input-group">
                                       <span class="input-group-text px-6" id="basic-addon1"><i
                                             class="ti ti-building-community fs-6"></i></span>
                                       <div style="flex-grow:1">
                                          <select name="partner_id[]" id="partner_id_{{$loop->index}}"
                                             class="select2-normal form-select">
                                             <option value="">-- Pilih Partner --</option>
                                             @foreach ($partners as $partner)
                                                <option value="{{$partner->id}}"
                                                   {{$partner->id == old('partner_id[' . $loop->index . ']', $codproduct?->partner_id) ? 'selected' : ''}}>{{$partner->name . ', ' . $partner->address}}</option>
                                             @endforeach
                                          </select>
                                       </div>
                                    </div>
                                    ...
                                    <div class="mt-2">
                                       ...
                                       <input type="text" name="price_buy[]" id="price_buy_{{ $item['id'] }}"
                                          class="form-control bg-white form-control-sm input-rupiah price-sale-input"
                                          data-id="{{ $item['id'] }}" value="{{ formatRupiah($priceBuy) }}" min="0" {{ $cod->is_finished != 0 ? 'disabled' : '' }} />
                                 ...
                                 <div class="">
                                    ...
                                    <div class="input-group">
                                       <span class="input-group-text px-6" id="basic-addon1"><i
                                             class="ti ti-align-justified fs-6"></i></span>
                                       <textarea class="form-control bg-white ps-2" name="desc[]" id="desc_{{$loop->index}}"
                                          cols="20" rows="2"
                        ...
                        <div class="pt-3 mt-3 border-top border-2">
                           ...
                           <div class="d-flex align-items-center mb-2 gap-2 justify-content-between">
                              ...
                              <div class="d-flex align-items-center gap-2">
                                 <input type="number" name="tax" value="{{ old('tax', $cod->tax) ?? setting('site.ppn') }}"
                           ...
                           <div class="d-flex align-items-center gap-2 mt-2">
                              <button type="button" id="refetchBtn"
                                 ...
                                 title="Muat Ulang Pemrosesan Produk" {{ $cod->is_finished != 0 ? 'disabled' : '' }}><i
                                    class="ti ti-refresh"></i></button>
                              <button type="submit" class="btn btn-primary w-100" {{ $cod->is_finished != 0 ? 'disabled' : '' }}>Simpan dan Lanjut ke
                                 Pembayaran</button>
                           </div>
                        </div>
                     </div>
         ...
         <div class="step-content" data-step="3" style="display: none;">
            ...
            <form action="{{route('customer-order.confirmPayment', $cod->id)}}" method="POST"
               enctype="multipart/form-data">
               @csrf
               <div class="row">
                     ...
                     <div class="card rounded-5">
                        <div class="card-body">
                           <div class="mb-4">
                              ...
                              @endif
                              <img src="{{ $cod->payment_receipt ? '/storage/' . $cod->payment_receipt : '' }}"
                                 id="preview-image" alt="Product Image Preview"
                                 class="{{ $cod->payment_receipt ? '' : 'd-none' }} rounded-4 shadow w-100" style="">
                           </div>
                           ...
                           @foreach($payments as $payment)
                              <div class="form-check mb-3">
                                 <input class="form-check-input" type="radio" name="payment_id"
                                    id="payment_{{ $payment->id }}" value="{{ $payment->id }}" {{ $cod->payment_id == $payment->id ? 'checked' : '' }} {{ $cod->is_finished != 0 ? 'disabled' : '' }}>
                                 <label class="form-check-label d-flex gap-3 align-items-start"
                                    for="payment_{{ $payment->id }}" style="cursor: pointer !important">
                                    @if($payment->type === 'bank')
                                       <img
                                          src="{{ $payment->image ? '/storage/' . $payment->image : '/assets/images/logos/bank.png' }}"
                                          alt="{{ $payment->bank_name }}" style="width:48px;height:48px;object-fit:contain;">
                                       <div>
                                    ...
                                    @elseif($payment->type === 'qris')
                                       <div class="w-100">
                                          <div class="d-flex align-items-center gap-2 w-100">
                                             <img
                                                src="{{ $payment->image ? '/storage/' . $payment->image : '/assets/images/logos/bank.png' }}"
                                                alt="QRIS" style="width:64px;height:64px;object-fit:contain;">
                                             ...
                                             <button type="button" data-id="{{$loop->index}}"
                                                class="btn btn-secondary btn-sm btn-show-qris ms-auto"
                                                style="white-space: nowrap;">Lihat QR</button>
                                          </div>
                                          <img
                                             src="{{ $payment->image ? '/storage/' . $payment->image : '/assets/images/logos/bank.png' }}"
                                             class="w-100 d-none" id="showQRIS-{{$loop->index}}" alt="QRIS"
               ...
               <div class="pb-4 position-sticky bottom-0">
                  <button type="button" class="btn btn-primary w-100" data-bs-toggle="modal"
                     data-bs-target="#confirmPaymentModal" {{ $cod->is_finished != 0 ? 'disabled' : '' }}>
                     Konfirmasi Pembayaran
                  </button>

               </div>
               <!-- Payment Confirmation Modal -->
               <div id="confirmPaymentModal" class="modal fade" tabindex="-1" aria-labelledby="danger-header-modalLabel"
                  aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
                     <div class="modal-content p-3 modal-filled bg-warning">
                        ...
                        <div class="modal-body" style="width: fit-content; white-space:normal">
                           ...
                           <p class="text-white">Pastikan telah mengecek dan memastikan pembayaran diterima dengan baik!
                              Karena transaksi tidak bisa diedit.</p>
                        </div>
                        <div class="modal-footer">
                           <button type="button" class="btn btn-light" data-bs-dismiss="modal">
                              Close
                     ...
                     <div class="btn-accordion p-3 border border-2 rounded-3 border-dashed">
                        <div class="d-flex align-items-center flex-wrap gap-3 justify-content-between"
                           ...
                           <div>
                              ...
                              <h6 class="fw-semibold fs-2 text-{{ $status[$pdebt->is_paid]['color'] }} mb-1" style="">
                                 {{ $status[$pdebt->is_paid]['label'] }}</h6>
                           </div>
                     ...
                     <div class="btn-accordion-content row mt-3">
                        <div class="col-md-6 mb-3 mb-md-0">
                           <div>
                              <div class="d-flex align-items-center" style="width:15em">
                                 <img
                                    src="{{ $pdebt->partner?->image ? asset('/storage/' . $pdebt->partner?->image) : '/assets/images/profile/user-1.jpg' }}"
                                    class="rounded-2" alt="partner Image {{ $pdebt->partner?->name }}" style="width: 4em" />
                                 <div class="ms-3">
                                    <h6 class="fw-semibold mb-1" style="white-space: normal !important">
                                       {{ $pdebt->partner?->name }}
                                    </h6>
                                    <div class="fw-normal text-muted" style="white-space:normal; font-size:13px; ">
                                       {{ $pdebt->partner?->description ?? 'Tidak ada deskripsi' }}
                                    </div>
                                 </div>
                              </div>
                           </div>
                           ...
                           <div>
                              <div class="fw-normal fs-2" style="white-space:normal; font-size:13px; ">
                                 {{ $pdebt->partner?->address }}
                              </div>
                              <div class="text-primary fs-2">{{ $pdebt->partner?->city }}.
                                 {{ $pdebt->partner?->postal_code }}
                              </div>
                           </div>
                           @if($pdebt->is_paid && $pdebt->image)
                              <div class="mt-2">
                                 <a href="{{'/storage/' . $pdebt->image}}" target="_blank" class="btn btn-secondary"><i
                                       class="ti ti-file me-2"></i> Bukti Pembayaran</a>
                              </div>
                           @endif
                        </div>
                        <div class="col-md-6 mb-3 mb-md-0">
                        ...
                        <div class="col-12 mt-3">
                           @php
                              ...
                              $priceBuy = $codProduct->price_buy ?? 0;
                              $subtotal = $priceBuy * $codProduct->qty; // Hitung subtotal awal
                           @endphp
                           <div class="border border-1 border-dashed border-primary mb-2 p-3 rounded-3">
                              <div class="d-flex align-items-center gap-2 mb-2 {{ $loop->index == 0 ? '' : 'mt-3' }}">
                                 <img
                                    src="{{ $codProduct?->product?->image ? '/storage/' . $codProduct?->product?->image : 'https://placehold.co/300?text=' . $codProduct?->product?->name }}"
