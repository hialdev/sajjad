               ...
               <nav aria-label="breadcrumb">
                  <ol class="breadcrumb">
                     <li class="breadcrumb-item">
                        <a class="text-muted text-decoration-none" href="{{ route('request-order.index') }}">Permintaan
                           Client</a>
                     </li>
      ...
      <div class="row">
         <div class="col-6 col-md-3  mb-auto d-flex align-items-center gap-2">
            @php
             ...
             @endphp
            <a href="{{ route('request-process.add', ['reqorder_id' => $reqorder->id]) }}"
               class="btn btn-primary {{$reqorder->status == '2' ? 'd-none' : ''}}"><i class="ti ti-atom-2 me-1"></i><span
                  class="d-none d-sm-inline-block">Proses</span></a>
            @if($reqorder->status == '2' && !$reqorder->invoice)
               <button data-bs-toggle="modal" data-bs-target="#generateInvoiceModal-{{$reqorder->id}}"
                  class="btn btn-secondary" style=""><i class="ti ti-file-invoice"></i> Buat Invoice</button>
            @endif
            <div>
               <div class="fw-normal fs-1 text-muted" style="">Status</div>
               <h6 class="fw-semibold fs-2 text-{{ $status[$reqorder->status]['color'] }} mb-1" style="">
                  {{ $status[$reqorder->status]['label'] }}</h6>
            </div>
                     ...
                     <div class="modal-content">
                        ...
                        <div class="modal-body pt-0">
                           <form action="{{ route('request-order.generate', $reqorder->id) }}" method="POST">
                              @csrf
                              <p class="text-muted" style="white-space: normal">Membuat penagihan / Invoice ke Client Secara
                                 Keseluruhan dari Request Order {{$reqorder->code}} ? <strong>Invoice akan dibuat untuk Client
                                    {{$reqorder->client->name}}</strong>. Akan gagal apabila terdapat Invoice Partial pada
                                 permintaan ini.</p>
                              <div class="d-flex gap-1 align-items-center justify-content-end">
                                 <button type="submit" class="btn btn-primary w-100">Ya, Buat Invoice</button>
                              </div>
                           </form>
                        </div>
                     </div>
         ...
         <div class="col-6 col-md-3 mb-4 d-flex justify-content-end align-items-start gap-2">
            <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteRequestOrder-{{$reqorder->id}}"><i
                  class="ti ti-trash"></i> <span class="ms-1 d-none d-md-inline-block">Hapus</span></button>
            @if($reqorder->invoice)
               <a href="{{route('request-order.getInvoice', ['id' => $reqorder->invoice->id])}}" class="btn btn-danger"
               ...
               <div class="modal-content p-3 modal-filled bg-danger">
                  ...
                  <div class="modal-body" style="width: fit-content; white-space:normal">
                     ...
                     <p class="text-white">Segala data yang berkaitan dengan Request Order tersebut juga akan dihapus
                        secara permanen. Penghapusan dapat dilakukan jika tidak ada Transaksi penting yang terkait.</p>
                  </div>
      ...
      <div id="stepper-form">
                  ...
                  <div class="card-body">
                     <div class="row">
                        <div class="col-md-4">
                           ...
                           <div>
                              ...
                              <h6 class="fw-semibold text-success mb-1" style="">
                                 {{ \Carbon\Carbon::parse($reqorder->date)->format('d F Y') }}
                              </h6>
                           </div>
                           <div>
                           ...
                           <div>
                              ...
                              <div class="fw-normal fs-2" style="white-space:normal; font-size:13px; ">
                                 {{ $reqorder->description ?? 'Tidak ada' }}
                              </div>
                           </div>
                        </div>
                        ...
                        <div class="col-md-4">
                           ...
                           <a href="{{route('client.setting', $reqorder->client->id)}}" target="_blank"
                        ...
                        <div class="col-md-4">
                           ...
                           <div>
                              ...
                              <h6 class="fw-semibold fs-2 text-{{ $statusInvoice[($reqorder->is_partial ?? $reqorder->generate_invoice)]['color'] }} mb-1"
                                 style="">
                                 {{ $statusInvoice[($reqorder->is_partial ?? $reqorder->generate_invoice)]['label'] }}</h6>
                           </div>
                           <hr>
                           <h6>Timestamp</h6>
               ...
               <div class="col-md-4 h-100 mb-3">
                  @if (isPdf($reqorder->attachment))
                     <div id="placeholder-file" class="">
                        <a href="{{ $reqorder->attachment ? '/storage/' . $reqorder->attachment : '#' }}" target="_blank"
                           id="preview-file-link"
                           class="d-flex align-items-center gap-2 p-2 rounded-2 border border-al-primary" target="_blank">
                           <i class="ti ti-file fs-6"></i>
                           <div id="preview-file-text" class="fs-2 line-clamp line-clamp-2">
                              {{ $reqorder->attachment }}
                           </div>
                        </a>
                        <embed src="{{ asset('storage/' . $reqorder->attachment) }}" class="rounded-4 overflow-hidden mt-3"
                           width="100%" height="600px" type="application/pdf">
                     </div>
                  @elseif(isImage($reqorder->attachment))
                     <img src="{{ $reqorder->attachment ? '/storage/' . $reqorder->attachment : '#' }}" id="placeholder-file"
                        alt="Raw Material Image Preview" class="rounded-4 shadow w-100" style="">
                  @else
                     <div id="placeholder-file" class="">
                        <a href="{{ $reqorder->attachment ? '/storage/' . $reqorder->attachment : '#' }}" target="_blank"
                           id="preview-file-link"
                           ...
                           <i class="ti ti-file fs-6"></i>
                           <div id="preview-file-text" class="fs-2 line-clamp line-clamp-2">
                              {{ $reqorder->attachment ?? 'Tidak ada lampiran' }}
                           </div>
                        </a>
                        <embed src="{{ asset('storage/' . $reqorder->attachment) }}"
                           class="{{ $reqorder->attachment ? '' : 'd-none' }} rounded-4 overflow-hidden mt-3" width="100%"
                  ...
                  <div id="preview-file" class="d-none">
                     ...
                     <embed id="preview-file-embed" src="" class="rounded-4 overflow-hidden mt-3" width="100%"
                        height="600px" type="application/pdf">
                  </div>
               </div>
               <div class="col-md-8">
                  <div class="card">
                     <div class="px-4 py-3 border-bottom">
                        ...
                        <button id="btnCloseData" class="btn btn-sm btn-danger position-absolute top-0 end-0 m-3"><i
                                 ...
                                 <div style="flex-grow:1">
                                    <select name="client_id" id="client_id" class="select2-normal form-select">
                                       ...
                                       @foreach ($clients as $client)
                                          <option value="{{ $client->id }}" {{ $client->id == old('client_id', $reqorder?->client_id) ? 'selected' : '' }}>
                                             {{ $client->name }}
                                          </option>
                                       @endforeach
                                    </select>
                           ...
                           <div class="mb-3">
                              ...
                              <div class="input-group">
                                 ...
                                 <input type="text" name="no_refrence"
                                    value="{{ old('no_refrence', $reqorder?->no_refrence) }}" class="form-control ps-2"
                        ...
                        <div class="col-12 col-sm-6 col-md-4 mb-2">
                           <div class="card rounded-4 h-100 overflow-hidden">
                              <div class="card-body p-0 h-100 d-flex flex-column">
                                 <img
                                    src="{{ $product->image ? '/storage/' . $product->image : 'https://placehold.co/160x90?text=' . $product->name }}"
                                    alt="Image {{ $product->name }}" class="d-block w-100 mb-2 bg-primary-subtle"
                                    style="aspect-ratio:16/9; object-fit:contain;">
                                 <div class="p-1 h-100 d-flex flex-column justify-content-between px-3">
                                    <div class="mb-1">
                                       <div class="d-inline-block p-1 px-2 rounded-2 bg-primary-subtle text-primary fs-2">
                                    ...
                                    @if ($product->type->type == "meteran")
                                       <div class="fs-2 fw-bold mt-1 text-muted"><i class="ti ti-clock"></i> Tersisa
                                          {{ $product->stock_count }} qty panjang utuh (qty berubah seiring inputan panjang)
                                       </div>
                                    @else
                                       <div class="fs-2 fw-bold mt-1 text-muted"><i class="ti ti-clock"></i> Tersisa
                                          {{ $product->stock_count }} qty</div>
                                    @endif
                                    <div class="d-flex mt-auto align-items-center gap-2 mb-3">
                                       ...
                                       <form action="{{ route('request-order.addCart', $reqorder->id) }}" method="POST"
               ...
               <div class="col-md-5 mb-3 order-first">
                  ...
                  <form action="{{ route('request-order.product.store', $reqorder->id) }}" method="POST"
                     ...
                     <div class="border border-2 border-dashed border-dark-subtle rounded-4 p-3">
                        ...
                        @forelse ($carts as $item)
                           ...
                           <div id="cart-item-{{ $item['id'] }}">
                              <div
                                 ...
                                 <img
                                    src="{{ $cart->image ? '/storage/' . $cart->image : 'https://placehold.co/300?text=' . $cart->name }}"
                                 ...
                                 <div>
                                    ...
                                    <div class="text-muted fs-2 mb-2">
                                       {{ $cart->description ?? 'tidak ada deskripsi' }}
                                    </div>
                                 </div>
                              ...
                              <div class="d-flex align-items-start gap-2">
                                 <div class="box-qty">
                                    <label for="qty" class="form-label {{ $cart->unit ? '' : 'text-danger'}} fs-2">
                                       {{ $cart->type->type == "meteran" ? "Dibeli sebanyak (Qty)" : ($cart->unit ? 'Dibeli Sebanyak (' . $cart?->unit?->code . ')' : 'Satuan produk belum diatur') }}</label>
                                    <div class="d-flex align-items-center gap-2">
                                       <input type="hidden" name="product_id[]" value="{{ $item['id'] }}">
                                       <input type="number" name="qty[]" id="qty_{{ $item['id'] }}"
                                          class="form-control form-control-sm qty-input" data-id="{{ $item['id'] }}"
                                          value="{{ $reqproduct->qty ?? $item['qty'] }}" min="1" {{$reqorder->status != 0 ? 'disabled' : ''}} />
                                    </div>

                                    <div class="fs-1 mt-1 stock-info text-muted">Stock tersisa <span
                                          class="text-count">{{ $cart->stock_count }}</span> qty</div>
                                 </div>
                                 @if($cart->type->type == 'meteran')
                                    <div>
                                       <label for="length" class="form-label {{ $cart->unit ? '' : 'text-danger'}} fs-2">
                                          {{ $cart->unit ? 'Panjang (cm) per Qty' : 'Satuan produk belum diatur' }}</label>
                                       <div class="d-flex align-items-center gap-2">
                                          <input type="number" name="length[]" id="length_{{ $item['id'] }}"
                                             class="form-control form-control-sm length-input" data-id="{{ $item['id'] }}"
                                             value="{{ $item['length'] ?? ($reqproduct?->length ?? 0) }}" min="1"
                                             max="{{ $cart->width }}" {{$reqorder->status != 0 ? 'disabled' : ''}} />
                                       </div>
                                 ...
                                 <div class="flex-grow-1">
                                    <label for="price_sale" class="form-label fs-2">Dengan Harga Jual {{ $cart->type->type =="meteran" ? "/m" :  ""}}</label>
                                    <input type="text" name="price_sale[]" id="price_sale_{{ $item['id'] }}"
                                       class="form-control form-control-sm input-rupiah price-sale-input"
                                       data-id="{{ $item['id'] }}" value="{{ formatRupiah($priceSale) }}" min="0"
                                       {{$reqorder->status != 0 ? 'disabled' : ''}} />
                                 ...
                                 <button class="btn btn-sm btn-danger remove-cart align-self-center"
                                    ...
                                    data-product-id="{{ $item['id'] }}" data-reqorder-id="{{ $reqorder->id }}"
                                    {{$reqorder->status != 0 ? 'disabled' : ''}}>
                        ...
                        @empty
                           <div class="text-center fs-3 p-5 border border-2 border-dashed border-primary rounded-4">
                        ...
                        <div class="pt-3 mt-3 border-top border-2">
                           ...
                           <div class="d-flex align-items-center mb-2 gap-2 justify-content-between">
                              ...
                              <div class="d-flex align-items-center gap-2">
                                 <input type="number" name="tax"
                                    value="{{ old('tax', $reqorder->tax) ?? setting('site.ppn')}}" id="tax"
                                    placeholder="0" style="width: 5em;" class="form-control text-center form-control-sm">
                                 %
                              </div>
                           </div>
                           ...
                           <div class="d-flex align-items-center gap-2 mt-2">
                              <button type="button" id="refetchBtn"
                                 ...
                                 title="Muat Ulang Pemrosesan Produk" {{$reqorder->status != 0 ? 'disabled' : ''}}><i
         ...
         <div class="step-content" data-step="3" style="display: none;">
            ...
            <div class="d-flex mb-3 align-items-center gap-3">
               ...
               <div class="d-flex align-items-center justify-content-center bg-primary text-white p-2 rounded-circle"
                  style="aspect-ratio:1/1; width:2.5em; height:2.5em">{{ count($reqorder->files) }}</div>
               <div class="ms-auto">
                  <button data-bs-toggle="modal" data-bs-target="#addLampiranModal"
                     class="d-inline-flex align-items-center gap-2 btn btn-primary btn-al-primary">
                        ...
                        <div class="card-body p-3">
                           <div class="d-flex align-items-center">
                              <span class="flex-shrink-0"><i
                                    class="ti ti-{{$stateFile[getTypeFile($file->file)]['icon']}} {{$stateFile[getTypeFile($file->file)]['color']}} display-6"></i></span>
                              <a href="{{ $file->file ? asset('storage') . '/' . $file->file : '#'}}" target="_blank"
                                 class="ms-3 flex-grow-1">
                                 <h4 class="card-title text-dark mb-1 fs-3">{{$file->name}}</h4>
                                 <p class="mb-0 fs-2 text-muted fw-normal">
                                    <i
                                       class="ti ti-clock me-2"></i>{{ \Carbon\Carbon::parse($file->created_at)->format('d M Y H:i') }}
                                 </p>
                                 <p class="mb-0 fs-2 text-muted fw-normal">
                                    <i class="ti ti-file me-2"></i>{{ getExt($file->file) }}
                                 </p>
                              </a>
                              <button data-bs-toggle="modal" data-bs-target="#deleteLampiranModal-{{$file->id}}"
                     ...
                     <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
                        <div class="modal-content p-3 modal-filled bg-danger">
                           ...
                           <div class="modal-body" style="width: fit-content; white-space:normal">
                              ...
                              <p class="text-white">Segala data yang berkaitan dengan alamat tersebut juga akan dihapus secara
                                 permanen.</p>
                           </div>
                           <div class="modal-footer">
                              <button type="button" class="btn btn-light" data-bs-dismiss="modal">
                              ...
                              <form
                                 action="{{route('request-order.file.destroy', ['id' => $reqorder->id, 'file_id' => $file->id])}}"
                                 method="POST">
                                 @csrf
                                 @method('delete')
                                 <button type="submit" class="btn btn-dark">Ya, Hapus</button>
                  ...
                  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                     <div class="modal-content">
                        ...
                        <div class="modal-body">
                           <form action="{{ route('request-order.file.store', $reqorder->id) }}" method="POST"
                              enctype="multipart/form-data">
                              @csrf
                              <div class="mb-2">
         ...
         <div class="step-content" data-step="5" style="display: none;">
                     ...
                     <div class="btn-accordion p-3 border border-2 rounded-3 border-dashed">
                        <div class="d-flex align-items-center flex-wrap gap-3 justify-content-between"
                           ...
                           <div>
                              ...
                              <h6 class="fw-semibold fs-2 text-{{ $status[$roInvoice->payment_status]['color'] }} mb-1"
                                 style="">{{ $status[$roInvoice->payment_status]['label'] }}</h6>
                           </div>
                           <div class="flex-grow-1 text-end mb-2 d-flex align-items-center justify-content-end gap-2">
                              ...
                              <a href="{{route('request-order.invoice.show', $roInvoice->id)}}" class="btn btn-primary"><i
                                    class="ti ti-credit-card"></i></a>
                           </div>
                        </div>
                        <div class="d-flex align-items-center gap-2">
                           <div class="w-100">
                              <div class="progress mt-1">
                                 <div
                                    class="progress-bar progress-bar-striped text-bg-{{$roInvoice->payment_percentage && $roInvoice->payment_percentage == '100' ? 'success' : 'info' }} progress-bar-animated"
                                    role="progressbar" aria-valuenow="45" aria-valuemin="0" aria-valuemax="100"
                                    style="width: {{$roInvoice->payment_percentage ?? '0'}}%">
                                 </div>
                              </div>
                           </div>
                           <div class="fs-2" style="white-space: nowrap">
                              <strong>{{$roInvoice->payment_percentage ?? '0'}}%</strong> Dibayar</div>
                        </div>
                     </div>
                        ...
                        <div class="col-md-6 mb-3 mb-md-0">
                           ...
                           <div>
                              ...
                              <h6 class="fs-2 fw-semibold text-success mb-1" style="">
                                 {{ \Carbon\Carbon::parse($roInvoice->requestOrder->date)->format('d F Y') }}
                              </h6>
                           </div>
                           <div>
                              <div class="fw-normal fs-1 text-muted" style="white-space:normal;">Kode Permintaan
                           ...
                           <div>
                              <div class="fw-normal fs-1 text-muted" style="white-space:normal;">Referensi
                                 Permintaan (PO)</div>
                              <a href="{{ $roInvoice->requestOrder->attachment ? '/storage/' . $roInvoice->requestOrder->attachment : '#' }}"
                                 class="badge fs-2 mb-1 bg-primary-subtle text-primary border-primary">
                                 <i class="ti ti-file"></i>
                        ...
                        <div class="col-md-6 mb-3 mb-md-0">
                           <div class="mb-3">
                              ...
                              <button type="button"
                                 ...
                                 data-bs-target="#produkProcessedModal-{{$roInvoice->purchaseOrder ? $roInvoice->purchaseOrder->id : $roInvoice->id}}"><i
                                    class="fs-4 ti ti-package"></i>
                                 {{ count($roInvoice->purchaseOrder ? $roInvoice->purchaseOrder->products : $roInvoice->requestOrder->products) }}
                                 Produk</button>

                              <!-- List Product modal -->
                              <div class="modal fade "
                                 id="produkProcessedModal-{{$roInvoice->purchaseOrder ? $roInvoice->purchaseOrder->id : $roInvoice->id}}"
                                    ...
                                    <div class="modal-content">
                                       ...
                                       <div class="modal-body pt-0">
                                          @foreach ($roInvoice->product_qty_price['products'] as $productId => $productRo)
                                             ...
                                             @endphp
                                             <div
                                                class="border border-1 border-dashed border-primary {{$loop->index + 1 == count($roInvoice->purchaseOrder ? $roInvoice->purchaseOrder->products : $roInvoice->requestOrder->products) ? '' : 'mb-2'}} p-3 rounded-3">
                                                <div
                                                   class="d-flex align-items-center gap-2 mb-2 {{ $loop->index == 0 ? '' : 'mt-3' }}">
                                                   <img
                                                      src="{{ $productPo->product->image ? '/storage/' . $productPo->product->image : 'https://placehold.co/300?text=' . $productPo->product->name }}"
                                                      alt="Image Product {{ $productPo->product->name }} in Cart"
                                                      class="d-block rounded-2" style="width: 5em; height:5em; object-fit:cover">
                                                   <div>
                                                      <div class="text-decoration-none text-dark fs-3 fw-semibold">
                                                         {{ $productPo->product->name }}
                                                      </div>
                                                      <div class="text-muted fs-2 mb-2">
                                                   ...
                                                   <div
                                                      ...
                                                      <div class="fs-3 fw-bold subtotal">
                                                         {{ formatRupiah($productRo['total_price_sale']) }}
                                                      </div>
                                                   </div>
                                                </div>
                                          ...
                                          <div class="pt-3 mt-3 border-top border-2">
                                             <div class="d-flex align-items-center gap-2 justify-content-between">
                                                ...
                                                <div class="fs-4 fw-bold">
                                                   {{ formatRupiah($roInvoice->product_qty_price['total_price']) }}</div>
                                             </div>
                                             <div class="d-flex align-items-center gap-2 justify-content-between">
                           ...
                           <div>
                              ...
                              <h6 class="fs-2 fw-semibold text-secondary mb-1" style="">
                                 {{ $roInvoice->purchase_order_id ? formatRupiah($roInvoice->product_qty_price['total_price_taxed']) : formatRupiah($roInvoice->requestOrder->total_price_taxed) }}
                              </h6>
                           </div>
                           <div>
                              <div class="fw-normal fs-1 text-muted" style="white-space:normal;">Dibayarkan</div>
                              <h6 class="fs-2 fw-semibold text-success mb-1" style="">
                                 {{ formatRupiah($roInvoice->sum_paid_total) }}</h6>
                           </div>
                           <div>
                              ...
                              <h6 class="fs-2 fw-semibold text-danger mb-1" style="">
                                 {{ formatRupiah($roInvoice->remaining_payment) }}</h6>
                           </div>
                        </div>
                     </div>
                  </div>
            ...
            @else
               <div class="p-4 text-center border border-dash">
                  Belum ada Invoice, Buat Invoice secara partial pada Pembelian ke Principal, atau Secara keseluruhan jika
                  status Permintaan Client telah selesai.
               </div>
            @endif
...
@section('scripts')
   ...
   <script>
      document.addEventListener('DOMContentLoaded', function () {
         ...
         function updateSubtotalAndTotal() {
            ...
            document.querySelectorAll('.qty-input').forEach(input => {
               ...
               let lengthInput = document.getElementById(`length_${id}`); // Ambil input length
               let length = 0;
               let priceSaleInput = document.getElementById(`price_sale_${id}`);
               let priceSale = cleanRupiah(priceSaleInput.value);

               // Cek apakah input length ada (berarti produk tipe meteran)
               if (lengthInput) {
                  length = parseFloat(lengthInput.value) || 0;
                  // Terapkan rumus qty * (length/100) * price_sale untuk produk meteran
                  let calculatedQty = qty * (length / 100);
                  let subtotal = calculatedQty * priceSale;
                  document.getElementById(`subtotal_${id}`).textContent = formatRupiah(subtotal);
                  total += subtotal;
               } else {
                  // Untuk produk non-meteran, gunakan rumus standar qty * price_sale
         ...
         document.querySelectorAll('.qty-input, .price-sale-input, .length-input').forEach(input => {
      ...
      $(document).ready(function () {
         $('input[id^="length_"]').on('input', function () {
            const length = $(this).val();
            const productId = $(this).data('id');
            const stockInfoContainer = $(this).closest(`#cart-item-${productId}`).find('.stock-info');

            if (length < 1) {
               stockInfoContainer.html(`<span class="text-danger">Panjang tidak valid</span>`);
               return;
            }
               ...
               success: function (res) {
                  if (res.success) {
                     stockInfoContainer.html(`
                           <span class="text-success">${res.qty} stok tersedia</span>
                        `);
                  } else {
                     stockInfoContainer.html(`<span class="text-warning">${res.message}</span>`);
                  }
               },
               error: function (xhr) {
                  const msg = xhr.responseJSON?.message || "Terjadi kesalahan.";
                  stockInfoContainer.html(`<span class="text-danger">${msg}</span>`);
               }
            });
         });
      });
