...
<head>
    ...
    <title>{{ config('app.name', 'Sajjad Zamzami Carpet - Information System') }}</title>

    <!-- Favicon icon-->
    <link rel="shortcut icon" type="image/png" href="{{filePath(setting('site.favicon'))}}" />
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            window.copyToClipboard = function(element) {
              ...
              alert('"'+$(element).text()+'" berhasil di copy');
            }
        });
    </script>
    ...
    function seePDF(bladePath, start_date, end_date){
        let url = "{{ route('pdf.preview') }}";
        let token = "{{ csrf_token() }}";

        previewPDF(url, token, bladePath, start_date, end_date);
    }
    </script>
    @yield('scripts')
</body>
